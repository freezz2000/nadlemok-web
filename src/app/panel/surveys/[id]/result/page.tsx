...
import type { AnalysisResult, Survey, Verdict } from '@/lib/types'
...
export default function PanelResultPage() {
  ...
  const [myResponses, setMyResponses] = useState<Record<string, number | string>>({})
  ...
  async function load() {
    ...
    const { data: surveyData } = await supabase
      .from('surveys')
      .select('*, project:projects(id, status, product_name, panel_size, satisfaction_threshold, panel_source)')
      .eq('id', id)
      .single()

    if (!surveyData) { router.push('/panel'); return }
    setSurvey(surveyData)

    const proj = (surveyData as unknown as {
      project: {
        id: string; status: string; product_name: string
        panel_size: number; satisfaction_threshold: number; panel_source: string
      }
    }).project

    setProjectStatus(proj?.status ?? null)
    setProductName(proj?.product_name ?? '')
    setPanelSize(proj?.panel_size ?? 50)
    setSatThreshold(proj?.satisfaction_threshold ?? 3.0)
    setPanelSource((proj?.panel_source as 'internal' | 'external' | 'mixed') ?? 'internal')
    ...
    if (responseRow) {
      ...
      if (responseRow.responses) {
        for (const [k, v] of Object.entries(responseRow.responses)) {
          restored[k] = v as number | string
        }
      }
      if (responseRow.open_weakness) restored['open_weakness'] = responseRow.open_weakness
      if (responseRow.open_improvement) restored['open_improvement'] = responseRow.open_improvement
      setMyResponses(restored)
    }

    // 분석 결과 (completed 상태일 때만)
    if (proj?.status === 'completed' && proj?.id) {
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto text-center py-20">
        ...
        <p className="text-text-muted text-sm">결과를 불러오는 중입니다...</p>
      </div>
    )
  }
  ...
  const n = result?.summary.total_responses ?? 0
  ...
  const hasKS      = (result?.kill_signals.length ?? 0) > 0
  ...
  const vc = result ? (VERDICT_CONFIG[result.verdict] ?? VERDICT_CONFIG['CONDITIONAL GO']) : null
  ...
  return (
    <div className="max-w-4xl mx-auto pb-16">
      ...
      {/* ── 헤더 ── */}
      <div className="mb-8">
        <p className="text-xs text-text-muted uppercase tracking-widest mb-1">설문 결과 확인</p>
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm p-6 mb-6">
        <div className="mb-5">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold text-text-muted uppercase tracking-widest">내 답변</span>
          </div>
          <h2 className="text-xl font-bold text-text">내가 제출한 답변</h2>
          <p className="text-sm text-text-muted mt-0.5">설문에서 선택한 응답을 확인하세요</p>
        </div>

        {Object.keys(myResponses).length === 0 ? (
          <p className="text-sm text-text-muted py-4 text-center">제출한 답변이 없습니다.</p>
        ) : (
          <div className="space-y-5">
            {/* 척도 문항 */}
            {scaleQuestions.map((q, i) => {
              const val = myResponses[q.key] as number | undefined
              return (
                <div key={q.key}>
                  <div className="flex items-start gap-2 mb-2">
                    <span className="text-sm text-text-muted flex-shrink-0">{i + 1}.</span>
                  ...
                  <div className="grid grid-cols-4 gap-2 ml-5">
                    {[1, 2, 3, 4].map((v) => (
                      <div
                        key={v}
                        className={`py-2.5 px-2 rounded-lg text-xs text-center ${
                          val === v
                            ? 'bg-navy text-white font-medium'
                            : 'border border-border text-text-muted/50'
                        }`}
                      >
                        {q.scaleLabels?.[v - 1] || scaleLabels[v]}
                      </div>
                    ))}
                  </div>
                  {val === undefined && (
                    <p className="text-xs text-text-muted/60 ml-5 mt-1">응답 없음</p>
                  )}
                </div>
              )
            })}
            ...
            {choiceQuestions.map((q, i) => {
              const selected = myResponses[q.key] as string | undefined
              return (
                <div key={q.key}>
                  <div className="flex items-start gap-2 mb-2">
                    <span className="text-sm text-text-muted flex-shrink-0">{scaleQuestions.length + i + 1}.</span>
                    <p className="text-sm font-medium text-text flex-1">{q.label}</p>
                  </div>
            ...
            {/* 주관식 문항 */}
            {textQuestions.map((q) => {
              const textKey = q.key === 'open_weakness' ? 'open_weakness'
                : q.key === 'open_improvement' ? 'open_improvement'
                : q.key
              const val = myResponses[textKey] as string | undefined
              return (
                <div key={q.key}>
                  <p className="text-sm font-medium text-text mb-2">{q.label}</p>
                  <div className="ml-0 px-3 py-2.5 bg-surface border border-border rounded-lg text-sm text-text min-h-[60px]">
                    {val || <span className="text-text-muted/60">응답 없음</span>}
                  </div>
                </div>
              )
            })}
      ...
      {projectStatus === 'analyzed' && (
        <div className="bg-white rounded-2xl border border-amber-200 shadow-sm p-8 mb-6 text-center">
          ...
          <h3 className="text-lg font-bold text-text mb-2">분석 완료 — 검토 중</h3>
          <p className="text-sm text-text-muted leading-relaxed">
            분석이 완료되어 현재 나들목 전문가가 결과를 검토 중입니다.<br />
            검토가 완료되면 여기서 전체 리포트를 확인하실 수 있습니다.
          </p>
        </div>
      )}

      {/* completed: 전체 분석 결과 표시 */}
      {projectStatus === 'completed' && result && vc && (
        <>
          {/* 보고서 헤더 */}
          <div className="mb-8 pt-4 border-t border-border">
            ...
            <h2 className="text-2xl font-bold text-text">{productName}</h2>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                {result.marketing_guide!.message && (
                  <div className="md:col-span-2 bg-navy/5 border border-navy/15 rounded-xl p-4">
                    ...
                    <p className="text-sm text-navy/80 leading-relaxed italic">&ldquo;{result.marketing_guide!.message}&rdquo;</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {/* completed인데 결과가 없는 경우 */}
      {projectStatus === 'completed' && !result && (
        <div className="bg-white rounded-2xl border border-border shadow-sm p-8 text-center">
          <p className="text-sm text-text-muted">분석 결과를 불러올 수 없습니다.</p>
        </div>
      )}

      {/* 아직 분석 전 (testing/analyzing 등) */}
      {projectStatus && !['analyzed', 'completed'].includes(projectStatus) && (
        <div className="bg-surface rounded-2xl border border-border p-6 text-center">
          <p className="text-sm text-text-muted">설문이 완료된 후 분석 결과를 확인할 수 있습니다.</p>
        </div>
      )}
    </div>
  )
